...
using Iskedular.Core.Services; // To use UserService and SessionService
using Microsoft.Extensions.DependencyInjection; // To use IServiceProvider
using Iskedular.Models; // To reference User model and UserRole enum
                ...
                if (authenticatedUser != null)
                {
                    ...
                    if (authenticatedUser.Role == UserRole.Admin)
                    {
                        ...
                        if (!string.IsNullOrEmpty(initialDefaultPasswordHash) && authenticatedUser.PasswordHash == initialDefaultPasswordHash)
                        {
                            ...
                            // adminAccountsForm.SetLoggedInAdmin(authenticatedUser); // Removed: User is now handled by SessionService.
                        ...
                        else
                        {
                            ...
                            Form9 adminDashboard = _serviceProvider.GetRequiredService<Form9>(); // Form9 was retrieved from DI.
                            adminDashboard.Show();
                            this.Hide();
                        }
                    }
